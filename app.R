...
working_directory <- "/var/lib/weclo/temp/"
...
server <- function(input, output, session) {
  ...
  {
    # API-Schlüssel und URLs für Tiles
    api_key <- "fd340d67-e9e1-438b-bca5-d2fce5aacd4d"
    dark_url <- paste0("https://api.mapbox.com/styles/v1/mapbox/dark-v10/tiles/{z}/{x}/{y}?access_token=", api_key)
    light_url <- paste0("https://api.mapbox.com/styles/v1/mapbox/light-v10/tiles/{z}/{x}/{y}?access_token=", api_key)
    
    # Verwende ifelse, um sicherzustellen, dass die Bedingung genau einen Wert hat
    url <- ifelse(identical(input$dark_mode, "dark"), dark_url, light_url)
    layer_id <- ifelse(identical(input$dark_mode, "dark"), "dark_mode_layer", "light_mode_layer")
    
    # Wende addTiles auf das leafletProxy Objekt an
    addTiles(
      leafletProxy("map"),  # Das korrekte Kartenobjekt angeben
      urlTemplate = url,
      options = tileOptions(attribution = "© Mapbox © OpenStreetMap"),
      layerId = layer_id
    )}%>%
