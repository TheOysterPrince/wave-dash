...
packages <- c(
  ...
  "raster"               # creating geographic matrices
)


# installation of necessary packages, please start with a new R session for success
...
directory <- "C:/Users/Eike/OneDrive/Studium/03 Master/Masterarbeit" 
setwd(directory)


# api call function ------------------------------------------------------
  
# which version of phython is used?
#py_config()

# configure which python version to use
#use_python("C:/Users/Eike/anaconda3/python.exe")
...
degrees_to_direction <- function(degree) {
  if (is.na(degree)) {return("unkown")            # in case of Na's in the wind data
  } else 
    {
   ...
   tags$script(
     HTML("
       $(document).ready(function() {
         $('.btn-custom').attr('style', 'height: 40px !important'); # adjust hight of action button
       });
     ")
   )
  ),
          ...
          card(
            ...
            layout_column_wrap(
              ...
              airDatepickerInput(inputId = "custom_datepicker",
                                 ...
                                 multiple = 2,
                                 value = c("2022-01-01 00:00:00", "2022-01-05 00:00:00"),
                                 clearButton = TRUE,
                                 timepicker = TRUE,
                                 todayButton = TRUE,
                                 addon = "none",
...
server <- function(input, output, session) {
  ...
  # define selcted_bounds as reactive value
  ...
  observeEvent(input$dark_mode, {
    leafletProxy("map") %>%
      clearTiles() %>%  # Entfernt den aktuellen Tile-Layer
      addProviderTiles(
     ...
     addTiles(
        urlTemplate = paste0(
          "https://wmts.marine.copernicus.eu/teroWmts/?service=WMTS&version=1.0.0&request=GetTile&layer=GLOBAL_ANALYSISFORECAST_WAV_001_027/cmems_mod_glo_wav_anfc_0.083deg_PT3H-i_202311/VHM0&tilematrixset=EPSG:3857@3x&tilematrix={z}&tilerow={y}&tilecol={x}&time=",
          format(input$custom_datepicker[1], "%Y-%m-%dT%H:%M:%S.000Z")
        ),
  ...
  # text output under map
    ...
    if (!is.null(selected_bounds$bounds) && !anyNA(selected_bounds$bounds)) {
      ...
      HTML("Select a date or time period and click at two points on the map to select an area.")
    }
  })
  ...
  observeEvent(input$start_api_call, {
    ...
    # converting of matrix into list of mit x(lon), y(lat) und z(hight) components
  ...
  observe({
    ...
    if (is.null(input$seed) || input$seed == "") {
      set.seed(NULL)  # Seed zurücksetzen
    } else {
      # Generiere den Seed basierend auf dem Textinput
    ...
    for (i in 1:input$number_waves) {
      ...
      # Calculate the contribution of the current wave
      wave <- outer(x, y, function(x, y) {
        2 * amplitude * ((sin(dx * x * w + dy * y * w + phase) + 1) / 2)^k
      })
      
      # Add the current wave's contribution to the superposition
